...
public class CustomTask extends AsyncTask<String, Void, String> {
    ...
    protected String doInBackground(String... strings) {
        try {
            String str;
            URL url = new URL("http://" + server_ip + ":" + server_port + "/" + strings[2]);  // 어떤 서버에 요청할지(localhost 안됨.)
            ...
            if (strings[2] == "login") {
                sendMsg = sendMsg + "&token=" + strings[3];
            }
            ...
            if(conn.getResponseCode() == conn.HTTP_OK) {
                ...
                if (strings[2] == "login") {
                    ...
                    try {
                        MapsActivity.Auth_token = check.split(" ")[1];
                    } catch (ArrayIndexOutOfBoundsException e) {
                        MapsActivity.Auth_token = null;
                    }
                }
